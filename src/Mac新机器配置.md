...
- 打开 iterm2 setting
- 选择 一级 Tab `Keys` 二级 Tab `Hotkey`
- 单击`Create a hotkey window` 绑定快捷键 我是 `option+space`
- 勾选 Floating window
- 切换 一级 Tab `Profiles` 调整工作空间 bash 以及外观设置
  我是 上半屏占位 50% 加上 50% 透明度
...
配置完成后就能在指定`workspace`打开 shell 的时候自动切换到对应 node 工具链的版本了
...
禁用所有状态切换
禁用所有快捷功能
可以锁定输入法为中文,用来避免英文且中文输入法还是输入英文需要再按`shift`确定的情况
高级:
禁用所有输入拓展以及下方所有设置
