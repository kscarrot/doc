...
> [!NOTE]
> a **Set** if a collecttion of **element** from a specified of discourse.
> The collection of everything in the universe of discourse is called **universal set** ,denote by $\mathcal{U}$
> The expression of $x \in X$ denote the statement that x is an element of X
> We write $x \notin X$ to mean $\lnot (x \in X)$ ,that x is not an element of X
...
> [!NOTE]
> Lex X be a set. A **sebset** of X is a set U such that
>  $$\forall a,(a \in U \Rightarrow a \in X)$$
>  We write $$U \subseteq X$$ for the assertion that U is a subset of X
>  Additionallay, the notation $$U \nsubseteq X$$ means the U is not a sebset of X
>  and the notation $$U \subsetneqq X$$ means that U is a proper subset of X
...
> [!NOTE]
> Let X and Y be sets. The (pairwise) **intersection** of X and Y, denoted  
> $$X \cap Y$$
...
> [!NOTE]
> Let X and Y be sets. The (pairwise) **union** of X and Y, denoted  
> $$X \cup Y$$
...
> [!NOTE]
> Let X and Y be sets. The (pairwise) **relative complement** of X and Y, denoted  
> $$X - Y$$
...
> [!NOTE]
> Let X and Y be sets. The (pairwise) **cartesian product** of X and Y, denoted  
> $$X \times Y$$
> $$ X \times Y = \{ (a,b) : a \in X \land b \in Y \}$$
> 

the elements $(a,b) \in X \times Y$ are called **ordered pairs** ,then
